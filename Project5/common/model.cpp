...
void indexVBO(
    ...
    for (int i = 0; i < static_cast<int>(in_vertices.size()); i++) {
        ...
        if (in_tangents.size() != 0)tangents = in_tangents[i];
        ...
        if (found) { // A similar vertex is already in the VBO, use it instead !
            ...
            if (in_tangents.size() != 0) out_tangents[index]+= in_tangents[i];
            ...
            if (in_tangents.size() != 0) out_tangents.push_back(tangents);
...
Drawable::Drawable(string path) {
    if (path.substr(path.size() - 3, 3) == "obj") {
        loadOBJWithTiny(path.c_str(), vertices, uvs, normals, tangents, VEC_UINT_DEFAUTL_VALUE);
...
Drawable::Drawable(const vector<vec3>& vertices, const vector<vec2>& uvs,
                   const vector<vec3>& normals, const vector<vec3>& tangents) : vertices(vertices), uvs(uvs), normals(normals),tangents(tangents) {
    createContext();
}
...
void Drawable::createContext() {
    ...
    indexVBO(vertices, uvs, normals, tangents,indices, indexedVertices, indexedUVS, indexedNormals,indexedTangents);
